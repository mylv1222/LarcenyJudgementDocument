...
def extractInformation(text,code,nn): #统筹抽取信息的函数，为了更细化的结构，会调用其他模块的函数
    import timeProcess
    import partThing
    import re
    flag=-1
    ...
    for line in lines:#将一行视作一次作案经过
        ...
        if(len(money)>0):
            ...
            for mm in money:
                tmp=re.search('(?:(?P<Cash>[0-9，,\.零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟亿]+)[余多]?元)',mm)
                if(tmp):
                    tmp=tmp.group('Cash')
                    print('\t'+tmp+'元',end="")
            print("")
...
def divideText(file): #将文书大致分为六个部分，方便后续处理，在特定的部分抽取信息，可以减少干扰因素
    import re
